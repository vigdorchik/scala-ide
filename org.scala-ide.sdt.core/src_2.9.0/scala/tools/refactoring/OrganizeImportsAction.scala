...
class OrganizeImportsAction extends RefactoringAction with ActionWithNoWizard {
  ...
  override def run(action: IAction) {
    ...
    def addImports(imports: Iterable[TypeNameMatch], pm: IProgressMonitor) {
      ...
      for {
        editor <- EditorHelpers.currentEditor orElse{ Tracer.println("currentEditor not found") }
        scalaSourceFile <- EditorHelpers.scalaSourceFile(editor)
        textSelection <- EditorHelpers.selection(editor)
      } { 
